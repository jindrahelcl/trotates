(() => {
  ...
  const TILE_MAX_SIZE = 192; // matches --tile-size default in CSS
  const GRID_GAP      = 3;   // matches gap: 3px on #grid
  const MAIN_PADDING  = 32;  // matches padding: 16px on main (both sides)
  ...
  function render(cols) {
    const available = window.innerWidth - MAIN_PADDING - (GRID_GAP * (cols - 1));
    const tileSize = Math.min(TILE_MAX_SIZE, Math.floor(available / cols));
  ...
  const lbPanelLocation = document.getElementById('lb-location');
  const lbPanelGlobal   = document.getElementById('lb-global');
  const lbPanelCampaign = document.getElementById('lb-campaign');
  ...
  document.querySelectorAll('.lb-tab').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (tab === 'location') show(lbPanelLocation); else hide(lbPanelLocation);
      if (tab === 'global')   show(lbPanelGlobal);   else hide(lbPanelGlobal);
      if (tab === 'campaign') show(lbPanelCampaign);  else hide(lbPanelCampaign);
  ...
  // Anon prompt — open account modal (pendingSolve will be handled after registration)
  document.getElementById('anon-save-btn').addEventListener('click', () => openAccountOverlay(true));
